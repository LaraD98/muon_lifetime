...
public:
    ...
    /**
     * @brief reset reset the histogram with a fixed bin width. Note that the lower bound in this case will be assumed as 0.
     * @param width The width of each bin
     */
    void reset(T width);

    /**
     * @brief  reset reset the histogram between two values.
     * @param lower The lower bound of the histogram
     * @param upper The upper bound
     */
    void reset(T lower, T upper);
...
template <std::size_t N, typename T, typename C>
void histogram<N, T, C>::reset(T width)
{
    m_lower  = 0;
    m_upper = width * N;
    m_width = width;
...
template <std::size_t N, typename T, typename C>
void histogram<N, T, C>::reset(T lower, T upper)
{
    m_lower = lower;
    m_upper = upper;
    m_width = (upper - lower) / static_cast<T>(N);
